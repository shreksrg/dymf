function ChineseCalendar(dateObj){
    ...
    this.solarDay2 = function(){
        ...
        tt += this.cDay(0, lDObj.day);
        lDObj = null;
        return tt;
    };

    this.solarDay1 = function(){
        var sDObj = new Date(this.SY, this.SM, this.SD);
        var lDObj = this.Lunar(sDObj);
        var tt = '';
        tt += '农历：';
...
$(function(){
    $(".datenum span[class='hide_time']").each(function(){
        var _dateObj = new Date($(this).html() * 1000);
        var _date = new ChineseCalendar(_dateObj);
        $(this).prev().html(_date.solarDay2());
    });


    function ontitle(e){
        if ($("span.hide_title", this).html().length > 0) {
            var _dateObj = new Date($("span.hide_time", this).html() * 1000);
            var _date = new ChineseCalendar(_dateObj);
            var _week = '';

            switch (_dateObj.getDay()) {
                case 0:
                    _week = "星期日";
                    break;
                case 1:
            ...
            var _timeStr = "今天是：" + (_dateObj.getMonth() + 1) + "月" + _dateObj.getDate() + "日 <br />" + _date.solarDay1() + "<br/>" + _week;
            $("div", $("#tip")).eq(0).html(_timeStr);
            $("div", $("#tip")).eq(1).html($("span.hide_title", this).html());
            $('#tip').show();
            var top = e.pageY + 7;
            var left = e.pageX + 7;
            $('#tip').css({
    ...
    $(".datenum").hover(ontitle, function(){
        $('#tip').hide();
    })
